...
async function cariSasaranGangDiPeta(
  page: Page,
  ...
  return page.evaluate(
    async ({ dataUrl, warna, toleransi, jarak, batas }) => {
      ...
      const piksel = konteks.getImageData(
        0,
        0,
        kanvas.width,
        kanvas.height,
...
async function pilihSegmenGang(page: Page): Promise<void> {
  await expect
    .poll(async () => (await cariSasaranGangDiPeta(page)).length, {
      timeout: 60000,
    })
    .toBeGreaterThan(0);
  ...
  throw new Error(
    "tidak ada segmen gang yang terpilih dari klik pada garis gang yang tergambar",
  );
}

async function balasUsulan(page: Page): Promise<void> {
  await page.route("**/api/koreksi", async (route) => {
    const badan = route.request().postDataJSON() as {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ok: true,
        proposal: {
          ...USULAN_TERSTRUKTUR.proposal,
          segmentId: badan.segment.segmentId,
        },
      }),
    });
  });
...
test.describe("koreksi lapangan berbantuan AI", () => {
  test("segmen gang dapat diklik dan panelnya menampilkan lebar serta panjang", async ({
    page,
  }) => {
    ...
    const panel = page
      ...
      .filter({ hasText: "Segmen gang terpilih" });
  ...
  test("kalimat bebas menghasilkan usulan terstruktur yang wajib disetujui manusia", async ({
    ...
    await page.fill(
      "#kalimat-koreksi",
      "gang ini sebenarnya cuma dua meter karena ada warung permanen",
    );
    ...
    await expect(usulan).toContainText(
      "gang ini sebenarnya cuma dua meter karena ada warung permanen",
    );
  ...
  test("menyetujui usulan mengubah kelas akses dan menandai sumber nilai", async ({
    page,
  }) => {
    await balasUsulan(page);
    await bukaLembarKerja(page);
  ...
  test("usulan yang tidak sesuai skema ditolak, bukan dipaksakan", async ({
    page,
  }) => {
    await page.route("**/api/koreksi", async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ok: true,
          proposal: { segmentId: 1, reason: "tanpa lebar" },
        }),
      });
    });
    await bukaLembarKerja(page);
    ...
    await expect(
      page
        .getByRole("status")
        .filter({
          hasText: "Jawaban layanan koreksi tidak dapat dibaca dan ditolak",
        }),
    ).toBeVisible();
    await expect(
      page.locator("article").filter({ hasText: "Lebar usulan" }),
    ).toHaveCount(0);
    await expect(panel).toHaveText(sebelum ?? "");
  });
    ...
    await page.route("**/api/koreksi", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          ok: false,
          message:
            "Layanan koreksi gagal merespons. Kalimat Anda tetap tersimpan, kirim ulang sebentar lagi.",
        }),
      }),
    );
    await bukaLembarKerja(page);
  ...
  test("kunci API tidak pernah sampai ke sisi klien", async ({ page }) => {
    ...
    page.on("request", (permintaan) => {
      ...
      if (permintaan.headers().authorization)
        permintaanKeluar.push(`berotorisasi ${url}`);
    });
